import { Controller, Get, Param, Query, DefaultValuePipe, ParseIntPipe, Post, Body, Inject } from '@nestjs/common';
...
export class PageAdminController {
  ...
  @AdminOnly()
    ...
    @Get('all')
    async findAll(
      @Query("is_active", new DefaultValuePipe(2), ParseIntPipe) is_active: number,
  ...
  @Post('add-update')
    async createOrUpdatePage(
      ...
      @Body("page_contents", new DefaultValuePipe([]))
      page_contents: {
        id: number;
    ...
    const updatedRecord = await this.pageService.createOrUpdatePage(
      ...
      page_contents.map((page_content, index) => {
        return {
          ...
          is_active:
            page_content.is_active == "true" || page_content.is_active == "on" ? true : false,
        };
      }),
    );
